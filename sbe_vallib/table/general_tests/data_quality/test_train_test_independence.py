...
def train_test_independence_test(train,
    ...
    params = train_test_independence_calculate(
        train, oos, feature_names, **params)

    gini_fact = params['logs']['split_stats']['gini_fact']
    gini_random_list = sorted(
        params['logs']['split_stats']['gini_random_list'])
    gini_quantile = min(sum(gini_random_list <= gini_fact) /
                        (len(gini_random_list) - 1), 1)
    ...
    result_test = make_test_report(
        table=result, color=result_color, title='train_test_independence_test')
    return result_test
