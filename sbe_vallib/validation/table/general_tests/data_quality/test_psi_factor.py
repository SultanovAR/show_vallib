...
def psi(pk, qk, base=None, axis=0):
    return entropy(pk, qk) + entropy(qk, pk)
...
def report_factor_psi(psi_of_feat: tp.Dict, threshold: tp.Tuple):
    ...
    semaphores = {feat: semaphore_by_threshold(
        psi_of_feat[feat]['psi'], threshold) for feat in psi_of_feat}
    
    res_df = psi_of_feat
    for feat in psi_of_feat:        
        res_df[feat]['feature'] = feat
        res_df[feat]['hist_train'] = [f'i:.3f' for i in psi_of_feat[feat]['hist_train']]
        res_df[feat]['hist_oos'] = [f'i:.3f' for i in psi_of_feat[feat]['hist_oos']]
...
def test_factor_psi(sampler,
    ...
    preprocessor = {'Discrete': Pipeline(('enc': encoder, 'quant': quantizer)),
                    'Continious': Pipeline(('quant': quantizer))}
    psi_of_feat = {}
    for col in interface.all_columns(x_oos):
        ...
        quant_x_train = preprocessor[feat_type].fit_transform(
            interface.get_columns(x_train, col))
        quant_x_oos = preprocessor[feat_type].transform(interface.get_columns(x_oos, col))

        bins = preprocessor[feat_type].named_steps['quant'].bins
        quant_x_train, _ = np.histogram(quant_x_train, bins=bins)
        quant_x_oos, _ = np.histogram(quant_x_oos, bins=bins)
        psi_of_feat[col] = {
            'psi': psi(quant_x_train, quant_x_oos),
            'feat_type': feat_type,
            'bin_count': len(bins),
            'hist_train': quant_x_train,
            'hist_oos': quant_x_oos
        }

    return report_factor_psi(psi_of_feat)
