...
import numpy as np
import pandas as pd
from utility import load_dump, save
import math
...
def psi_factor_classes_test(X_train,
    ...
    """
    feature_types = get_feature_types(X_train, discr_uniq_val=discr_uniq_val, discr_val_share=discr_val_share)
    result = []
    ...
    result = [pd.Series(feature_types, name="Feature_type").reindex(X_train.keys())] + result
    ...
    for feature in tqdm(X_train.columns):
        ...
        if feature_types[feature] == 'Discrete':
            if len(set(np.unique(X_train_dropna.tolist())).intersection(np.unique(X_test_dropna.tolist()))) == 0:
                ...
                result[feature] = discrete_binning(X_train_dropna, X_test_dropna, min_bin_size=min_bin_size)
        else:
            result[feature] = continuous_binning(X_train_dropna, X_test_dropna, min_bin_size=min_bin_size)

    result = pd.DataFrame(result).T[['psi', 'feature_type', 'bins_cnt', 'bins_train', 'bins_test']]
...
def get_feature_types(df, discr_uniq_val=10, discr_val_share=0.8):
    ...
    for feature in df.columns:
        ...
        unique_counts = np.unique(X_train_dropna.tolist(), return_counts=True)[1]
...
def discrete_binning(X_train, X_test, min_bin_size=0.05):
    ...
    """
    unique_values, unique_counts = np.unique(X_train.tolist(), return_counts=True)
    # Sort values by counts descending
    unique_values = [x for _, x in sorted(zip(unique_counts, unique_values), reverse=True)]
    unique_counts = sorted(unique_counts, reverse=True)
    ...
    bins_test = np.unique([bins_train[x] for x in X_test if x in bins_train.keys()], return_counts=True)
    bins_test = {bin_: count for bin_, count in zip(bins_test[0], bins_test[1])}
    ...
    final_bins_train = np.unique([bins_train[x] for x in X_train if x in bins_train.keys()], return_counts=True)
    final_bins_train = {bin_: count / len(X_train) for bin_, count in zip(final_bins_train[0], final_bins_train[1])}

    final_bins_test = np.unique([bins_train[x] for x in X_test if x in bins_train.keys()], return_counts=True)
    final_bins_test = {bin_: count / len(X_test) for bin_, count in zip(final_bins_test[0], final_bins_test[1])}
...
def continuous_binning(X_train, X_test, min_bin_size=0.05):
    ...
    while binning_possible:
        ...
        for j in range(len(bin_points) - 1):
            X_train_bin = X_train[(X_train > bin_points[j]) & (X_train <= bin_points[j + 1])]

            if len(X_train_bin) / 2 >= min_bin_size * len(X_train):
                median = np.median(X_train_bin)
                test_left = ((X_test > bin_points[j]) & (X_test <= median)).sum()
                test_right = ((X_test > median) & (X_test <= bin_points[j + 1])).sum()
    ...
    for i in range(len(bin_points) - 1):
        train_share = ((X_train > bin_points[i]) & (X_train <= bin_points[i + 1])).sum() / len(X_train)
        test_share = ((X_test > bin_points[i]) & (X_test <= bin_points[i + 1])).sum() / len(X_test)
